  ...
  export default {
    data() {
      const d = {
        ...
        listId: 0,
        refresh() {
          this.lists[10].vals = val2binaryArr(this.lists[11].val)
          this.lists[11].vals = val2binaryArr(this.lists[11].val)
          this.lists[12].vals = val2binaryArr(this.lists[0].val)
        }
      }
...
<template>
  ...
  <div class="tableItem">
    ...
    <li><span>{{listId}}</span></li>
    <li v-for="list in lists" :key="list.id">
      <el-input-number v-if="!list.mutil && list.range" size="mini" @change="handleChange" type="number" :min=list.range[0] :max=list.range[1] v-model="list.val"> 
      </el-input-number>
      <el-select v-if="!list.mutil && list.selecs" size="mini" v-model="list.val" placeholder="请选择">
        <el-option
